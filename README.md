...
- `/models` – This folder contains pre-trained models used by the bot.
    - `linear_model_5d_price.pkl` – Regression model for predicting 5-day high/low/average price.
...
- `/research` – Notebooks and prototype scripts for modeling and experimental analysis.
    - `Prediction_model.ipynb` –  This notebook documents the experimentation and selection process for the price prediction models used in the trading bot. The workflow includes:
        - Classification Models: 
          - Tested approaches like Logistic Regression and Random Forest to classify market movements (e.g., up/down recorded as 1/0, to invest or not).
        - Regression Models:  
        ...
        - Model Selection Rationale: 
          - After benchmarking all models and considering interpretability, speed, and generalization, the notebook concludes that a **multi-output linear regression model** is the best fit for the trading bot due to its simplicity, transparency, and strong real-world performance in out-of-sample testing.
...
- `finbert_utils.py` –  Utility module for performing sentiment analysis on news headlines or articles using the FinBERT model.
...
- **Feature Engineering**  
    - The price prediction model uses a carefully selected set of engineered features for each asset.
    - The final predictors chosen for forecasting the 5-day average, minimum, and maximum prices are:
...
```
> Notes: 
> - **No live trading is enabled by default**. Live and paper trading can be enabled by switching broker configuration, but this has not yet been tested in this version. The bot currently runs in backtesting mode by default.
...
- In-depth review of all trades made during backtesting (saved in `/outputs/multi_stock/trade_ledger.csv` and `/outputs/single_stock/trade_ledger.csv`).
